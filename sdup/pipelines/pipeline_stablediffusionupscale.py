# Adapted from: https://github.com/huggingface/diffusers/blob/main/src/diffusers/pipelines/stable_diffusion/pipeline_stable_diffusion_upscale.py
...
class StableVideoDiffusionUpscalePipeline(
    ...
    def check_inputs(
        ...
        if isinstance(image, list) or isinstance(image, torch.Tensor) or isinstance(image, np.ndarray):
            ...
            if image_batch_size % prompt_batch_size:
                raise ValueError(
                    f"`prompt` has batch size {prompt_batch_size} and `image` has batch size {image_batch_size} or not in Video."
        ...
        return image_batch_size // prompt_batch_size 
    ...
    def __call__(
        ...
        padding: Optional[str] = 'zero',
        latent_padding_mode: Optional[str] = 'zero',
    ):
        r"""
        ...
        # 1. Check inputs
        num_samples_ = self.check_inputs(
            prompt,
            image,
            noise_level,
            callback_steps,
        ...
        if weight_dtype is not None:
            self.unet = self.unet.to(dtype=weight_dtype)
        ...
        if num_samples is None:
            ...
            if num_samples != num_samples_:
                assert num_samples % num_samples_ == 0
                print(f"Using {padding} padding...")
                if padding == 'zero':
                    div_num = (num_samples - num_samples_) // (num_samples_ - 1)
                    print(f"image: {image.shape}")
                    padded_image = []
                    for i in range(image.shape[0] - 1):
                        padded_image.append(image[i])
                        for j in range(div_num):
                            padded_image.append(torch.zeros_like(image[0]))

                    padded_image.append(image[-1])
                    ...
                    if latents is not None:
                        ...
                        for i in range(latents.shape[0] - 1):
                            padded_latents.append(latents[i])
                            for j in range(div_num):
                                padded_latents.append(torch.zeros_like(latents[0]))

                        padded_latents.append(latents[-1])
        ...
        latents_num = num_samples * num_images_per_prompt
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                latent_model_input = rearrange(latent_model_input, "(b f) c h w -> b c f h w", f=num_samples)
                ...
                noise_pred = rearrange(noise_pred, "b c f h w -> (b f) c h w", f=num_samples)
    ...
    def ddim_inversion(
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                latents = self.next_step(noise_pred, t, latents, self.scheduler)
